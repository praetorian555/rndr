...
bool Rndr::Shape3DRenderer::Render(f32 delta_seconds, CommandList& command_list)
{
    ...
    command_list.CmdUpdateBuffer(m_model_transform_buffer, Opal::AsBytes(m_instances));
    ...
    command_list.CmdDrawIndicesMulti(m_pipeline, PrimitiveTopology::Triangle, Opal::ArrayView<DrawIndicesData>(m_draw_commands));
...
void Rndr::Shape3DRenderer::DrawCube(const Matrix4x4f& transform, const Vector4f& color)
{
    DrawShape(ShapeType::Cube, transform, color);
}

void Rndr::Shape3DRenderer::DrawSphere(const Matrix4x4f& transform, const Vector4f& color)
{
    DrawShape(ShapeType::Sphere, transform, color);
}
...
void Rndr::Shape3DRenderer::DrawShape(ShapeType shape_type, const Matrix4x4f& transform, const Vector4f& color)
{
    ...
    m_draw_commands.PushBack(draw_data);
}