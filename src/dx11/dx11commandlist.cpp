...
void rndr::CommandList::ClearColor(Image* Image, math::Vector4 Color) const
{
...
void rndr::CommandList::ClearDepth(Image* Image, real Depth) const
{
...
void rndr::CommandList::ClearStencil(Image* Image, uint8_t Stencil) const
{
    if (Image == nullptr || Image->DX11DepthStencilView == nullptr)
    {
...
void rndr::CommandList::ClearDepthStencil(Image* Image, real Depth, uint8_t Stencil) const
{
    if (Image == nullptr || Image->DX11DepthStencilView == nullptr)
    {
...
void rndr::CommandList::BindShader(Shader* Shader) const
{
...
void rndr::CommandList::BindImageAsShaderResource(Image* Image, int Slot, Shader* Shader) const
{
    assert(Shader);
    assert(Image);
...
void rndr::CommandList::BindSampler(Sampler* Sampler, int Slot, Shader* Shader) const
{
    assert(Shader);
    assert(Sampler);
...
void rndr::CommandList::BindBuffer(Buffer* Buffer, int Slot, Shader* Shader) const
{
    if (Shader != nullptr)
    {
        switch (Shader->Props.Type)
...
void rndr::CommandList::BindFrameBuffer(FrameBuffer* FrameBuffer) const
{
    ...
    {
        RenderTargetViews[Index] = FrameBuffer->ColorBuffers[Index]->DX11RenderTargetView;
    }
...
void rndr::CommandList::BindInputLayout(InputLayout* InputLayout) const
{
    DX11DeferredContext->IASetInputLayout(InputLayout->DX11InputLayout);
}

void rndr::CommandList::BindRasterizerState(RasterizerState* State) const
{
    DX11DeferredContext->RSSetState(State->DX11RasterizerState);
}

void rndr::CommandList::BindDepthStencilState(DepthStencilState* State) const
{
    DX11DeferredContext->OMSetDepthStencilState(State->DX11DepthStencilState,
                                                State->Props.StencilRefValue);
}

void rndr::CommandList::BindBlendState(BlendState* State) const
{
    constexpr uint32_t kUpdateAllSamples = 0xFFFFFFFF;
    DX11DeferredContext->OMSetBlendState(State->DX11BlendState, nullptr, kUpdateAllSamples);
}

void rndr::CommandList::DrawIndexed(PrimitiveTopology Topology, int IndicesCount) const
{
