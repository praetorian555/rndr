...
5. **Layered rendering.** `VkRenderingInfo::layerCount` is hardcoded to one (command-buffer.cpp) and there is
   ...
   with a geometry shader writing `SV_RenderTargetArrayIndex`, read back by layer.
...
8. **A resolve at the end of the pass.** `CmdResolveTexture` is a standalone transfer; a tiled device wants
   ...
   texture back, the way the multisampled draw case reads its standalone resolve.
...
else. Item 8 only if it goes multisampled, item 9 only on a tiled device.
...
needs 1, 3, 4 and 6; the mesh-shader path needs 2 as well.
